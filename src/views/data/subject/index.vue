<template>
    <a-layout class="flex-layout">
      <Breadcrumb />
      <a-card :title="$t('议题管理')" class="general-card">
        <a-row>
          <a-col :flex="62">
            <a-form
        ...
        <a-space :size="'medium'">
          <a-button type="primary" @click="NewSubject()">
            <template #icon>
              <icon-plus />
            </template>
            新增
  ...
  <script lang="ts" setup>
    ...
    const EditSubject = async (pk: number) => {
      buttonStatus.value = 'edit';
      operateRow.value = pk;
      drawerTitle.value = t('编辑');
      await fetchSubjectDetail(pk);
      openNewOrEdit.value = true;
    };
    const DeleteSubject = () => {
    ...
    const submitNewOrEdit = async () => {
      ...
      try {
        if (buttonStatus.value === 'new') {
          ...
          await updateSubject(operateRow.value, form);
          cancelReq();
    ...
    const submitDelete = async () => {
      setLoading(true);
      try {
        await deleteSubject({ pk: rowSelectKeys.value });
        cancelReq();
    ...
    // 请求API列表
    const fetchSubjectList = async (params: SubjectParams = {}) => {
      setLoading(true);
      try {
    ...
    // 请求部门详情
    const fetchSubjectDetail = async (pk: number) => {
      setLoading(true);
      try {
    ...
    const onPageChange = async (current: number) => {
      await fetchSubjectList({ page: current, size: pagination.pageSize });
    };
  
    // 事件: 分页大小
    const onPageSizeChange = async (pageSize: number) => {
      pagination.pageSize = pageSize;
      await fetchSubjectList({ page: 1, size: pageSize });
    };

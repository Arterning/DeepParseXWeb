<template>
    <a-layout class="flex-layout">
      ...
      <a-card :title="$t('标签管理')" class="general-card">
        ...
        <a-space :size="'medium'">
          <a-button type="primary" @click="NewTag()">
            <template #icon>
              <icon-plus />
            </template>
            新增
        ...
        <div class="content">
          ...
          >
            ...
            <template #operate="{ record }">
              <a-space>
                <a-link @click="EditTag(record.id)">
                  编辑
                </a-link>
              </a-space>
            </template>
  ...
  <script lang="ts" setup>
    ...
    const EditTag = async (pk: number) => {
      buttonStatus.value = 'edit';
      operateRow.value = pk;
      drawerTitle.value = t('编辑');
      await fetchTagDetail(pk);
      openNewOrEdit.value = true;
    };
    const DeleteTag = () => {
    ...
    const submitNewOrEdit = async () => {
      ...
      try {
        if (buttonStatus.value === 'new') {
          ...
          await updateTag(operateRow.value, form);
          cancelReq();
    ...
    const submitDelete = async () => {
      setLoading(true);
      try {
        await deleteTag({ pk: rowSelectKeys.value });
        cancelReq();
    ...
    // 请求部门详情
    const fetchTagDetail = async (pk: number) => {
      setLoading(true);
      try {
    ...
    const onPageChange = async (current: number) => {
      await fetchTagList({ page: current, size: pagination.pageSize });
    };
  
    // 事件: 分页大小
    const onPageSizeChange = async (pageSize: number) => {
      pagination.pageSize = pageSize;
      await fetchTagList({ page: 1, size: pageSize });
    };

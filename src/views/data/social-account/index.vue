<template>
    <a-layout class="flex-layout">
      <Breadcrumb />
      <a-card :title="$t('社交账号')" class="general-card">
        <a-row>
          <a-col :flex="62">
            <a-form
  ...
  <script lang="ts" setup>
    ...
    import {
      createSocialAccount,
      deleteSocialAccount,
      querySocialAccountDetail,
      querySocialAccountList,
      SocialAccountParams,
      SocialAccountReq,
      SocialAccountRes,
      updateSocialAccount,
    ...
    // 表格
    const renderData = ref<SocialAccountRes[]>([]);
    ...
    const EditSocialAccount = async (pk: number) => {
      buttonStatus.value = 'edit';
      operateRow.value = pk;
      drawerTitle.value = t('编辑');
      await fetchSocialAccountDetail(pk);
      openNewOrEdit.value = true;
    };
    ...
    const submitNewOrEdit = async () => {
      ...
      try {
        if (buttonStatus.value === 'new') {
          ...
          await fetchSocialAccountList();
        } else {
          await updateSocialAccount(operateRow.value, form);
          cancelReq();
    ...
    const submitDelete = async () => {
      setLoading(true);
      try {
        await deleteSocialAccount({ pk: rowSelectKeys.value });
        cancelReq();
    ...
    // 请求API列表
    const fetchSocialAccountList = async (params: SocialAccountParams = {}) => {
      setLoading(true);
      try {
        const res = await querySocialAccountList(params);
    ...
    // 请求部门详情
    const fetchSocialAccountDetail = async (pk: number) => {
      setLoading(true);
      try {
        const res = await querySocialAccountDetail(pk);
        resetForm(res);
      } catch (error) {
        // console.log(error);
      } finally {
    ...
    const onPageChange = async (current: number) => {
      await fetchSocialAccountList({ page: current, size: pagination.pageSize });
    };
  
    // 事件: 分页大小
    const onPageSizeChange = async (pageSize: number) => {
      pagination.pageSize = pageSize;
      await fetchSocialAccountList({ page: 1, size: pageSize });
    };
    ...
    const search = async () => {
      await fetchSocialAccountList({
        ...formModel.value,
      } as unknown as SocialAccountParams);
    };

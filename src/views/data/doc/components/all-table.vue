      ...
      <div class="content-modal">
        <a-modal :closable="false" :on-before-ok="beforeSubmit" :title="drawerTitle" :visible="openNewOrEdit"
          ...
          <a-form ref="formRef" :model="form">
            <a-form-item :feedback="true" :label="$t('data.doc.form.title')" :rules="[
              { required: true, message: $t('data.doc.form.title.help') },
            ]" field="title">
              <a-input v-model="form.title" :placeholder="$t('data.doc.form.name.placeholder')"></a-input>
            </a-form-item>
            <a-form-item :feedback="true" :label="$t('data.doc.form.name')" field="name">
        ...
        <a-modal v-model:visible="collectVisi" @ok="handleOk" @cancel="handleCancel">
          ...
          <div>
            <a-select :style="{ width: '100%', marginBottom: '15px' }" placeholder="请选择需要保存的收藏夹">
              <a-option>Beijing</a-option>
              <a-option>Shanghai</a-option>
              <a-option>Guangzhou</a-option>
            </a-select>

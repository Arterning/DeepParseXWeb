<template>
    <a-layout class="flex-layout">
      <Breadcrumb />
      <a-card :title="$t('黑料管理')" class="general-card">
        <a-row>
          <a-col :flex="62">
            <a-form
        ...
        <a-space :size="'medium'">
          <a-button type="primary" @click="NewScandal()">
            <template #icon>
              <icon-plus />
            </template>
            新增
        ...
        <div class="content">
          ...
          >
            ...
            <template #operate="{ record }">
              <a-space>
                <a-link @click="EditScandal(record.id)">
                  编辑
                </a-link>
                <a-link @click="router.push({name: 'DataDetail', params: { id: record.id }, query: { type: 'scandal' } })">
                  查看
                </a-link>
              </a-space>
            </template>
  ...
  <script lang="ts" setup>
    ...
    // 表格
    const renderData = ref<ScandalRes[]>([]);
    ...
    const EditScandal = async (pk: number) => {
      buttonStatus.value = 'edit';
      operateRow.value = pk;
      drawerTitle.value = t('编辑');
      await fetchScandalDetail(pk);
      openNewOrEdit.value = true;
    };
    const DeleteScandal = () => {
    ...
    const formDefaultValues: ScandalReq = {
      name: '',
    };
    const form = reactive<ScandalReq>({ ...formDefaultValues });
    ...
    const submitNewOrEdit = async () => {
      ...
      try {
        if (buttonStatus.value === 'new') {
          await createScandal(form);
          cancelReq();
          Message.success(t('submit.create.success'));
          await fetchScandalList();
        } else {
          await updateScandal(operateRow.value, form);
          cancelReq();
    ...
    const submitDelete = async () => {
      setLoading(true);
      try {
        await deleteScandal({ pk: rowSelectKeys.value });
        cancelReq();
    ...
    // 请求API列表
    const fetchScandalList = async (params: ScandalParams = {}) => {
      setLoading(true);
      try {
        const res = await queryScandalList(params);
    ...
    // 请求部门详情
    const fetchScandalDetail = async (pk: number) => {
      setLoading(true);
      try {
        const res = await queryScandalDetail(pk);
        resetForm(res);
      } catch (error) {
        // console.log(error);
      } finally {
    ...
    const onPageChange = async (current: number) => {
      await fetchScandalList({ page: current, size: pagination.pageSize });
    };
  
    // 事件: 分页大小
    const onPageSizeChange = async (pageSize: number) => {
      pagination.pageSize = pageSize;
      await fetchScandalList({ page: 1, size: pageSize });
    };

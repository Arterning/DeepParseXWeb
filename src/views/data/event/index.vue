<template>
  <a-layout class="flex-layout">
    <Breadcrumb />
    <a-card :title="$t('事件管理')" class="general-card">
      <a-row>
        <a-col :flex="62">
          <a-form
...
<script lang="ts" setup>
  ...
  const EditEvent = async (pk: number) => {
    buttonStatus.value = 'edit';
    operateRow.value = pk;
    drawerTitle.value = t('编辑');
    await fetchEventDetail(pk);
    openNewOrEdit.value = true;
  };
  const DeleteEvent = () => {
  ...
  const formDefaultValues: EventReq = {
    name: '',
  };
  const form = reactive<EventReq>({ ...formDefaultValues });
  ...
  const submitNewOrEdit = async () => {
    ...
    try {
      if (buttonStatus.value === 'new') {
        ...
        await updateEvent(operateRow.value, form);
        cancelReq();
  ...
  const submitDelete = async () => {
    setLoading(true);
    try {
      await deleteEvent({ pk: rowSelectKeys.value });
      cancelReq();
  ...
  // 请求API列表
  const fetchEventList = async (params: EventParams = {}) => {
    setLoading(true);
    try {
      const res = await queryEventList(params);
  ...
  // 请求部门详情
  const fetchEventDetail = async (pk: number) => {
    setLoading(true);
    try {
      const res = await queryEventDetail(pk);
      resetForm(res);
    } catch (error) {
      // console.log(error);
    } finally {
  ...
  const onPageChange = async (current: number) => {
    await fetchEventList({ page: current, size: pagination.pageSize });
  };

  // 事件: 分页大小
  const onPageSizeChange = async (pageSize: number) => {
    pagination.pageSize = pageSize;
    await fetchEventList({ page: 1, size: pageSize });
  };

  ...
  <div class="h-full flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto p-6">
      ...
      <div class="border-t border-gray-200 pt-6">
        <div
          v-if="isHtmlContent(mailData?.original)"
          class="prose max-w-none"
          v-html="mailData?.original"
        ></div>
        <div
          v-else
          class="whitespace-pre-wrap break-words dark:text-white text-gray-800 leading-relaxed"
        >
          {{ mailData?.original }}
        </div>
      </div>
      ...
      <!-- 附件信息 -->
      <div
        v-if="mailData?.attachments && mailData.attachments.length > 0"
...
<script setup lang="ts">
  ...
  function isHtmlContent(input: string): boolean {
    ...
    const hasHtmlTag =
      /<html[^>]*>/i.test(trimmed) && /<\/html>/i.test(trimmed);
    const hasHeadTag =
      /<head[^>]*>/i.test(trimmed) && /<\/head>/i.test(trimmed);
    const hasBodyTag =
      /<body[^>]*>/i.test(trimmed) && /<\/body>/i.test(trimmed);
    const hasDivTag = /<div[^>]*>/i.test(trimmed) && /<\/div>/i.test(trimmed);
    const hasTableTag =
      /<table[^>]*>/i.test(trimmed) && /<\/table>/i.test(trimmed);

    // 至少需要html标签或者doctype + html结构
    return (
      hasHtmlTag ||
      (hasDoctype && (hasHeadTag || hasBodyTag)) ||
      hasDivTag ||
      hasTableTag
    );
  }
